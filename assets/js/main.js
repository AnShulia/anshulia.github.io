...
function accordionPricePage(){
  /*
const sectionPrice = document.querySelectorAll(".page-price-section-accordion-section");
console.log(sectionPrice)
     ...
     sectionPrice.forEach((section) => {
      ...
      header.addEventListener("click", () => {
        ...
        // Закрываем все
        document.querySelectorAll(".page-price-section-accordion-section ul").forEach((ul) => {
          ul.style.height = "0px";
          ul.classList.remove("open");
        });
  ...
  sectionPrice.forEach((section, index) => {
    const header = section.querySelector("h4");
    const content = section.querySelector("ul");

    // Устанавливаем курсор для заголовка
    header.style.cursor = "pointer";
  ...
  // Обновляем высоту открытой секции при изменении размера окна
 ...
 const sectionPrice = document.querySelectorAll(".page-price-section-accordion-section");
  ...
  sectionPrice.forEach((section, index) => {
    ...
    if (index === 0) {
      content.classList.add("open");
      content.style.height = content.scrollHeight + "px";
      svg.classList.add("rotated"); // Добавляем класс для поворота
    }
    ...
    header.addEventListener("click", () => {
      ...
      document.querySelectorAll(".page-price-section-accordion-section ul").forEach((ul) => {
        ul.style.height = "0px";
        ul.classList.remove("open");
        // Убираем поворот у всех SVG
        document.querySelectorAll(".page-price-section-accordion-section svg").forEach((s) => {
      ...
      if (!isOpen) {
        content.classList.add("open");
        content.style.height = content.scrollHeight + "px";
        svg.classList.add("rotated"); // Поворачиваем SVG текущей секции
      }
    });
  });
